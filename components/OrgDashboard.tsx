...
import { Plus, Target, Users, CircleCheck as CheckCircle, ChartBar as BarChart2, Mail, Circle as XCircle, Clock, Briefcase, Loader as Loader2, CircleAlert as AlertCircle, Trash2, UserPlus, UserCheck, GraduationCap } from 'lucide-react';
...
const OrgDashboard: React.FC<OrgDashboardProps> = ({ onNavigate }) => {
  ...
  const [studentsToFollow, setStudentsToFollow] = useState<StudentToFollow[]>([]);
  const { user } = useAuth();
    ...
    try {
      ...
      if (matches) {
        const formattedCandidates: Candidate[] = matches.map((match: any) => ({
          ...
          avatar_url: match.profiles?.avatar_url || null,
  ...
  const fetchStudentsToFollow = async () => {
    if (!user) return;
    ...
    try {
      ...
      const { data } = await supabase
        .from('profiles')
        .select('id, name, avatar_url, professional_summary')
      ...
      if (data) {
        const students = data
          ...
          .map((u: any) => ({
            ...
            professional_summary: u.professional_summary,
            isFollowing: false
          }));
        setStudentsToFollow(students);
      }
    } catch (error) {
  ...
  const toggleFollowStudent = async (studentId: string, isCurrentlyFollowing: boolean) => {
    if (!user) return;
    ...
    try {
      if (isCurrentlyFollowing) {
        ...
        await supabase
          .from('connections')
          .insert({ follower_id: user.id, following_id: studentId });
      }

      setStudentsToFollow(studentsToFollow.map(s =>
        s.id === studentId ? { ...s, isFollowing: !isCurrentlyFollowing } : s
      ));
      fetchFollowCounts();
    } catch (error) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="h-16 bg-gray-900 relative">
            <div className="absolute -bottom-6 left-4 p-1 bg-white rounded-lg shadow-md">
               <img src={profile?.avatar_url || `https://picsum.photos/seed/${user?.id || 'org'}/100`} className="w-16 h-16 rounded-lg object-cover" alt="Org" />
            </div>
          </div>
      ...
      <div className="lg:col-span-3 space-y-6">
                 ...
                 <div key={candidate.id} className="group relative border border-gray-100 rounded-xl p-3 hover:shadow-md transition-all bg-white">
                   <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <img src={candidate.avatar_url || `https://picsum.photos/seed/${candidate.id}/50`} className="w-10 h-10 rounded-lg object-cover" alt="Profile" />
                        <div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 bg-black text-white flex items-center justify-between">
            ...
            <span className="text-[9px] font-black bg-white/10 px-2 py-0.5 rounded tracking-tighter uppercase">{studentsToFollow.length}</span>
          </div>
          <div className="p-4 space-y-3 max-h-[400px] overflow-y-auto">
            {studentsToFollow.length === 0 ? (
              <div className="text-center py-6">
                <GraduationCap className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">No students to follow</p>
              </div>
            ) : (
              studentsToFollow.map((student) => (
                <div key={student.id} className="flex items-center gap-3 group">
                  <img
                    src={student.avatar_url || `https://picsum.photos/seed/${student.id}/100`}
                    className="w-9 h-9 rounded-lg border border-gray-100 object-cover"
                    alt={student.name}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-black text-black leading-tight truncate">{student.name}</p>
                    <p className="text-[9px] font-bold text-gray-500 leading-tight uppercase truncate">Student</p>
                  </div>
                  <button
                    onClick={() => toggleFollowStudent(student.id, student.isFollowing)}
                    className={`text-[9px] font-black px-2 py-1 rounded uppercase hover:scale-105 transition-all flex items-center gap-1 ${
                      student.isFollowing ? 'text-[#facc15] bg-black' : 'text-black bg-[#facc15]'
                    }`}
                  >
                    {student.isFollowing ? (
