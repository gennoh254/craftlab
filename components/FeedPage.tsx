...
const FeedPage: React.FC<FeedPageProps> = ({ userRole, onNavigate }) => {
  ...
  const fetchNetworkUsers = async () => {
    ...
    try {
      ...
      const oppositeType = profile?.user_type === 'STUDENT' ? 'ORGANIZATION' : 'STUDENT';

      const { data: crossRoleUsers } = await supabase
        .from('profiles')
      ...
      const { data: sameRoleUsers } = await supabase
        .from('profiles')
        .select('id, name, user_type, avatar_url')
        .eq('user_type', profile?.user_type || 'STUDENT')
        .limit(10);

      const allUsers = [...(crossRoleUsers || []), ...(sameRoleUsers || [])];

      if (allUsers && user) {
        const seen = new Set<string>();
        const usersWithFollowStatus = allUsers
          .filter((u: any) => {
            if (u.id === user.id || followedIds.includes(u.id) || seen.has(u.id)) return false;
            seen.add(u.id);
            return true;
          })
          .slice(0, 6)
      ...
      <div className="lg:col-span-3 space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
           <h4 className="text-[10px] font-black text-black flex items-center gap-2 mb-4 uppercase tracking-widest">
             <Users className="w-3.5 h-3.5 text-[#facc15]" /> {userRole === UserRole.STUDENT ? 'Organizations' : 'Students'} & Networking
           </h4>
