/*
  ...
  ## Overview
  This migration adds comprehensive features for notifications, CV/resume uploads, 
  and enhanced user profile data. It also adds triggers for automatic notifications.

  ## New Tables
  
  ### `notifications`
  User notifications for opportunity updates and placement confirmations
  - `id` (uuid, primary key)
  - `user_id` (uuid, references profiles) - Recipient of notification
  - `type` (text) - Type: opportunity_update, placement_success, application_status
  - `title` (text) - Notification title
  - `message` (text) - Notification content
  - `opportunity_id` (uuid, optional) - Related opportunity
  ...
  ### `profiles`
  - Add `cv_url` field for uploaded resume/CV
  - Add `cv_uploaded_at` field for tracking CV uploads

  ## Security
  - RLS policies for notifications (users can only see their own)
  - Admin can send notifications to all users
  - Automatic notifications on opportunity and application changes
*/
...
-- Notifications policies: Admins can create notifications
CREATE POLICY "Admins can create notifications" ON notifications
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() IN (SELECT auth_user_id FROM admin_users));

-- Notifications policies: Users can update their own notifications (mark as read)
